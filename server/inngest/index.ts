...
export const inngest = new Inngest({ id: "groce-shop" })
...
const sendMonthlyOffers = inngest.createFunction(
    {
        ...
        const { deals, users } = await step.run("fetch-deals-and-users", async () => {
            // Get top discounted products as featued deals
        ...
        // Send in batches of 10 avoid overwhelming mail server
        const batchSize = 10;
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        if (["Cancelled", "Delivered"].includes(order.status as string)) return {
            skipped: true, reason: `Order is ${order.status}`
        }
        ...
        const availableRider = await prisma.deliveryPartner.findFirst({
            where: { isActive: true, id: { notIn: busyRidersIds as string[] } }
        })
